...
struct LockScreenWeatherWidget: View {
	...
	private var focusRowInCalendarSlot: some View {
		...
		.opacity(shouldShowFocusWidget ? 1 : 0)
		.accessibilityLabel("\(String(format: String(localized: "Focus active: %@"))) \(focusDisplayName)")
		.allowsHitTesting(false)
	}
	...
	private var combinedFocusCalendarRow: some View {
		...
		.accessibilityLabel("\(line). \(String(format: String(localized: "Focus active: %@"))) \(focusDisplayName)")
		.allowsHitTesting(false)
	}
