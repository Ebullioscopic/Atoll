...
- **Agent Tower context ring**: Each agent card now shows how full that session's context window is — amber past 70%, red past 90% — plus the session title the agent gave itself. The window size is calibrated from the tokens actually seen in the transcript rather than guessed from the model name, because a 1M-context session does not always say so in its model identifier; until it is certain, the raw token count is shown instead of a percentage.
...
- **A badge you can pick**: the mark shown beside your track while coding agents are running is now an emoji you choose — 🤖 by default, with a few presets and a field for any other. Emptying it goes back to the plain symbol.
