...
### Added
- **Teleprompter debrief**: Sections you marked `## !` tick off in green as you cover them, and when a take ends you get a short report — what you meant to cover but did not, your pace both overall and while actually speaking, how much of the script you really read, and the exact words where you went off script or skipped ahead. Anything that is not a problem is simply absent. The last twenty takes per script are kept for a month, on your Mac.
- **Teleprompter follows your voice**: Set Advance to "Follow my voice" and the prompter tracks the words you actually say, waiting when you pause, absorbing a skipped clause, and keeping up when you jump to another section. Recognition happens entirely on your Mac — no audio leaves it and none is saved. The language picker says up front which languages have an on-device model installed, since most do not until you add them in System Settings › Keyboard › Dictation, and Atoll declines rather than falling back to a server. The microphone is shared with the AI assistant by agreement, so the two can never record you at once.
...
- **Teleprompter reads your Keynote notes (experimental)**: Import the open deck and its presenter notes become a script, one section per slide, skipped slides left out. With "Follow the slideshow" on, the prompter moves to the right section as you change slides during a take — asked for only while a take is running, and never opening Keynote by itself. Notes that happen to start with `#` or `>` are escaped so they are read aloud instead of silently becoming prompter markup, and a `\` at the start of any line now means "this is prose". Marked experimental because it is built against Keynote's own scripting dictionary but has not been confirmed against a live slideshow.
...
- Fixed the Prompter tab not scrolling. It holds a scrollable script, but never told the notch to hold off its own close-on-scroll gesture while the pointer is over it, so every wheel event was eaten by the gesture instead of reaching the script — the same arrangement the clipboard, notes and terminal tabs already use.
