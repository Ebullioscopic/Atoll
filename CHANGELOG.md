...
### Fixed
- The notch header's glyphs are sized to look the same, rather than to use the same point size. Equal point size is equal cap height, which is not equal optical size: measured at 15pt, `gearshape` covers 289pt² of ink against `web.camera`'s 208 — 39% more — and `list.clipboard` stands 19pt tall against `timer`'s 16. Each symbol now has a point size solved so its ink height lands on 16pt, and they share a box wide enough not to clip the tall ones.
