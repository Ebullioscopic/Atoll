...
- Temperature now starts in whatever unit macOS itself displays, instead of always starting in Celsius. Read via `MeasurementFormatter` with a natural scale rather than the locale's measurement system, since the Temperature control in Language & Region is set independently — a metric locale can be set to Fahrenheit, and the UK is non-metric overall while still reporting weather in Celsius. Only the initial value is affected; a unit already chosen is left alone.
- Volume and brightness step sizes now explain why they are greyed out when an external display app owns those keys, instead of appearing broken.
