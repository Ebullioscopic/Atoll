...
### Fixed
- The Clipboard Manager description named a fixed shortcut, Cmd+Shift+V, which was not the default and could not have been right in any case — the shortcut is rebindable under Shortcuts. It now shows whichever combination is actually bound, updates when that changes, and says to assign one when none is set.
