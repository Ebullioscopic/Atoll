...
### Fixed
- Turning on lyrics no longer crowds the transport controls in the notch. With the calendar shown, lyrics render as a third line inside the player rather than in a side panel, and the block holding the title, artist and scrubber was a `GeometryReader` — which fills whatever space it is given, so the gap above the controls was only ever the leftover. A third line consumed it, closing the elapsed-time row up against the buttons. The block now sizes to its content and the gap is an explicit minimum.
