...
### Fixed
- The brightness HUD no longer stays dead for the whole session, and the native volume HUD no longer draws over this app's. Both come from the media key event tap: `CGEvent.tapCreate` fails outright until Accessibility is granted, and granting it does not relaunch the app, so a single attempt at startup left the keys reaching macOS untouched. Volume still looked fine because its HUD is also driven by a CoreAudio observer, but brightness has no such fallback — it is only detected when this app drives the change — so it showed nothing at all. The tap is now retried until it installs (every 2s for the first minute, then every 15s, giving up after an hour), instead of only being retried by the side effect of switching HUD style in Settings and switching back (#601).
