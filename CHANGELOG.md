...
### Fixed
- The native volume HUD no longer draws over this app's, and the brightness HUD no longer stays dead for the whole session. On macOS 26 the volume HUD is drawn by Control Center, not `OSDUIHelper`, so `SystemOSDManager`'s SIGSTOP suppresses a process that no longer renders it — the media key event tap is the only thing still holding the native HUD back. That tap was attempted exactly once at startup, and `CGEvent.tapCreate` fails outright until Accessibility is granted; since granting it does not relaunch the app, one denied prompt disabled interception for the rest of the session. The tap is now retried until it installs (every 2s for the first minute, then every 15s, giving up after an hour), instead of only being retried by the side effect of switching HUD style in Settings and switching back.
