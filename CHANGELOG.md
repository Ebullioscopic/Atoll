...
- **Performance (phase 4)**: Scoped notch re-renders (the non-notch screen check is cached instead of scanning every display on each body evaluation, and the stats manager no longer invalidates the whole view once per second) and deferred non-essential startup work (feature managers, default animations, app-icon application, welcome sound) off the first frame.
