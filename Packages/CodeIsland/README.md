...
- `CodeIslandCore` for the future provider-neutral, sanitized state contract.
...
The Core, Runtime, UI, and helper boundaries are deliberately inert. Imported
source and test candidates live under `Upstream` directories excluded by
SwiftPM; they must be adapted and verified before migration into a product
target.
