...
final class AudioEngine {
    ...
    private let logger = os.Logger(subsystem: Bundle.main.bundleIdentifier ?? "FineTune", category: "AudioEngine")
    ...
    private func applyTapOutputState(to tap: any ProcessTapControlling, for pid: pid_t, deviceUIDs: [String]? = nil) {
        ...
        if let primaryUID = resolvedUIDs.first {
            tap.updateDeviceEQSettings(settingsManager.getDeviceEQSettings(forDeviceUID: primaryUID))
        }
    ...
    // MARK: - Device EQ (global per output device)

    /// EQ applied to every app routed to the given output device, in addition
    ...
    func getDeviceEQ(forDeviceUID deviceUID: String) -> EQSettings {
        settingsManager.getDeviceEQSettings(forDeviceUID: deviceUID)
    }
    ...
    private func tapInitialState(forApp app: AudioApp, primaryDeviceUID: String, deviceVolume: Float) -> TapInitialState {
        ...
        return TapInitialState(
            ...
            deviceEQSettings: settingsManager.getDeviceEQSettings(forDeviceUID: primaryDeviceUID),
