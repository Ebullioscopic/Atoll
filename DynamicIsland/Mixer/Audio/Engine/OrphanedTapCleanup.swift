...
private let logger = os.Logger(subsystem: "com.finetuneapp.FineTune", category: "OrphanedTapCleanup")

/// Scans CoreAudio for orphaned AtollMixer aggregate devices and destroys them.
    ...
    static func destroyOrphanedDevices() {
        ...
        for device in devices {
            ...
            guard let name = try? device.readDeviceName(),
                  name.hasPrefix("AtollMixer-") else { continue }
        ...
        if destroyedCount == 0 {
            logger.info("[CLEANUP] No orphaned AtollMixer devices found")
        } else {
