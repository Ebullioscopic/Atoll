...
final class SettingsManager {
    ...
    private let logger = os.Logger(subsystem: Bundle.main.bundleIdentifier ?? "FineTune", category: "SettingsManager")
    ...
    struct Settings: Codable {
        ...
        var deviceEQSettings: [String: EQSettings] = [:]  // deviceUID → global device EQ (all apps on that device)
        ...
        init(from decoder: Decoder) throws {
            ...
            deviceEQSettings = try c.decodeIfPresent([String: EQSettings].self, forKey: .deviceEQSettings) ?? [:]
    ...
    init(directory: URL? = nil) {
        let baseDir = directory ?? FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!.appendingPathComponent("AtollMixer")
    ...
    // MARK: - Device EQ (global per output device, applies to every app routed there)

    func getDeviceEQSettings(forDeviceUID deviceUID: String) -> EQSettings {
        return settings.deviceEQSettings[deviceUID] ?? EQSettings.flat
    }

    func setDeviceEQSettings(_ eqSettings: EQSettings, forDeviceUID deviceUID: String) {
