...
- Codex 已完成数按“未查看完成”单独计数；点击 Codex 关闭态/完成提示进入任务页，或直接切换到 Codex 任务页时会立即清零并持久化，最近完成对话仍保留，后续新完成任务会重新计数。
...
- `CodexPresentationTests: PASS`：Codex 空闲时保留展开页标签和明确缺省态；关闭态固定为单行，进行中显示为 `数量 · 进行中`，存在未查看完成记录时只显示优先级更高的 `数量 · 已完成`，宽度和高度均不增加；展开页仍保留独立对话区块、状态、相关内容和逐条跳转元数据。
...
- 本次“查看即清零”已完成聚焦测试和 Debug 构建，但尚未启动 Debug App 执行真实鼠标点击验收；需在真实完成提示出现时确认进入 Codex 页后计数即时消失、新完成任务再次出现。
