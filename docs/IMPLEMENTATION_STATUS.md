...
- Codex 固定忙碌态、固定已完成态、计数文案和配色保持不变；临时 Sneak Peek 继续保持 3.5 秒展示窗口，重新编排入场、稳定展示和退出节奏，并以轻量缩放、模糊收敛、环形扫光和克制的微粒庆祝替代原有持续滚动提示。
- Codex 已完成数按“未查看完成”单独计数；点击 Codex 关闭态/完成提示进入任务页，或直接切换到 Codex 任务页时只清零未读徽标，已完成对话继续保留在历史托盘中。
...
- `CodexCompletionAcknowledgementTests: PASS`：查看后清零、重复确认幂等、最近完成历史保留、确认状态持久化、旧状态兼容，以及新完成任务重新计数。
...
- 2026-08-23 Codex 临时提示动效已通过 `CodexPresentationTests`，覆盖新对话与完成庆祝均保持 3.5 秒及标准展示模式；`CodexHookHelper` Debug target 与 Atoll Debug 主 App 均构建成功，运行进程路径和 Debug Bundle ID 已复验。
