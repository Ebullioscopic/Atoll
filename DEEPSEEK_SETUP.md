...
- 刘海默认在其他应用全屏时隐藏；鼠标移到刘海区域可临时唤出并正常点击，移开后自动收起，退出全屏后恢复。隐藏时不拦截网页的鼠标事件；设置中的全屏隐藏选项仍可调整。iBar 层级更新和桌面切换不会重新唤出已隐藏的刘海。
...
DeepSeek、Gemini、OpenAI、Claude 和 Groq 的 API Key 存入 macOS 钥匙串。启动时自动迁移旧偏好设置中的 Key，成功写入后才删除原值；若钥匙串不可用，设置页会提示错误并保留原值，解锁后重新打开模型设置即可重试。远程 DeepSeek 兼容接口必须使用 HTTPS 并提供 Key；只有回环地址 `localhost`、`127.0.0.1`、`[::1]` 可以使用 HTTP 或省略 Key。模型名称及接口能力依据 [DeepSeek 文档](https://api-docs.deepseek.com/)；可用性由账户和服务端决定。
...
测试覆盖钥匙串迁移与失败重试、打包身份校验、请求构造、图片生命周期、Markdown/部分流、传输/取消、缩放边界、pi 会话及升级回滚。原生界面验收使用隔离的 QA 应用配置和本地模拟服务，不消耗真实模型额度。真实付费 DeepSeek 回复没有作为本次验收的一部分。
